...
const productsResource = mesh.resource("products.list", {
  key: (filters: { search: string }) => ["products", filters],
  staleTime: "1m",
  async fetch(filters: { search: string }) {
    const products = await api.get<Product[]>("/products", { query: filters });
